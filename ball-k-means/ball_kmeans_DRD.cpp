...
ClusteringResults ball_k_means_Ring(MatrixOur& dataset, MatrixOur& centroids, bool detail = false) {
    ...
    //二维vector
    //cluster_point_index[i]中，记录的是ball i中的所有向量
    ClusterIndexVector cluster_point_index;
    //二维vector
    ClusterIndexVector clusters_neighbors_index;
    //二维vector
    //now_data_index[j]中，记录第j个annular area有哪些点
    ClusterIndexVector now_data_index;
    //二维vector
    //记录点到质心距离
    ClusterDistVector temp_dis;
    ...
    while (true) {
        ...
        if (new_centroids != old_centroids) {
            ...
            //应该是更新每个ball的半径
            //Update the radius of each cluster
            ...
            //A ﬂag corresponding to a ball cluster is used to judge whether a ball cluster is stable.
            flag.setZero();
            ...
            for (unsigned int now_ball = 0; now_ball < k; now_ball++) {
                ...
                //now_centers中记录的是neighbors的质心坐标
                ...
                //Theorem 5. Assume ball k-means is implemented on a given dataset D. Let C be a queried ball cluster.
                //If all the neighbor ball clusters of C are stable in one iteration, 
                //C will not take part in the distance calculations in the next iteration.
                //algorithm 3的第2行
                if (judge) {
                    continue;
                }
    ...
    clusteringResults.labels = labels;
    clusteringResults.cluster_point_index = cluster_point_index;
    return clusteringResults;
}

ClusteringResults ball_k_means_noRing(MatrixOur& dataset, MatrixOur& centroids, bool detail = false) {
    ...
    while (true) {
        ...
        if (new_centroids != old_centroids) {
            ...
            //get the radius of each cluster
    ...
    ClusteringResults clusteringResults;
    clusteringResults.labels = labels;
    clusteringResults.cluster_point_index = clusters_point_index;
    return clusteringResults;
...
MatrixOur load_data(const char* filename) {
    ...
    */
    ...
    //  x代表数据集中有多少个向量，epileptic有11500个向量
    //  y/x代表一个向量有多少维，因为x乘dimension等于y
...
update_centroids(MatrixOur& dataset, ClusterIndexVector& cluster_point_index, unsigned int k, unsigned int n,
    ...
    */
    ...
    for (unsigned int i = 0; i < k; i++) {
        ...
        //cluster_point_index[i]中记录的是ball i中的所有向量
...
//we ﬁrst search the neighbor clusters in the current iteration in the order of the neighbor clusters of the last iteration,
//then search any remaining neighbor clusters.
//In this way, the distances of the neighbor clusters to the queried cluster are almost ordered before the sort.
inline sortedNeighbors
get_sorted_neighbors_Ring(VectorOur& the_Rs, MatrixOur& centers_dis, unsigned int now_ball, unsigned int k,
                          vector<unsigned int>& now_center_index) {
...
ClusteringResults ball_k_means(MatrixOur& dataset, int k, bool isRing = false, bool detail = false,
    ...
    ClusteringResults clusteringResults;
    if (isRing) {
        clusteringResults = ball_k_means_Ring(dataset, centroids, detail);
    }
    else {
        clusteringResults = ball_k_means_noRing(dataset, centroids, detail);
    }
    return clusteringResults;
}
...
int main(int argc, char* argv[]) {
    ...
    int k=40;
    start_time = clock();
    //一行是一个向量，epileptic有179维，6*26+23
    MatrixOur dataset = load_data("/usr/dataLake/DRD/attributeVector.csv");
    ClusteringResults clusteringResults = ball_k_means(dataset, k, false, true);
    end_time = clock();
    cout << (double)(end_time - start_time) / CLOCKS_PER_SEC << endl;
    ofstream outfile;
    outfile.open("labels.csv");
